...
namespace ZeroX.UI.SafeAreaSystem
{
    public class SafeArea : MonoBehaviour
    {
        [System.Serializable]
        public class OffsetAnchorConfig
        {
            public bool use;
            public bool top;
            public bool bottom;
        ...
        [SerializeField] private OffsetAnchorConfig offsetAnchorConfig = new OffsetAnchorConfig();
        [SerializeField] bool Logging = false;  // Conform to screen safe area on Y-axis (default true, disable to ignore)

        Vector2 originAnchorMin = Vector2.zero;
        Vector2 originAnchorMax = Vector2.one;
        ...
        void ApplySafeArea (Rect r)
        {
            ...
            if (Screen.width > 0 && Screen.height > 0)
            {
                ...
                {
                    ApplyOffsetAnchor(ref anchorMin, ref anchorMax);
        ...
        private void ApplyOffsetAnchor(ref Vector2 anchorMin, ref Vector2 anchorMax)
        {
            if(offsetAnchorConfig.use == false)
                return;

            if (offsetAnchorConfig.left)
                anchorMin.x += originAnchorMin.x;
            else
                anchorMin.x = originAnchorMin.x;

            if(offsetAnchorConfig.right)
                anchorMax.x -= (1 - originAnchorMax.x);
            else
                anchorMax.x = originAnchorMax.x;
            
            if (offsetAnchorConfig.bottom)
                anchorMin.y += originAnchorMin.y;
            else
                anchorMin.y = originAnchorMin.y;

            if(offsetAnchorConfig.top)
                anchorMax.y -= (1 - originAnchorMax.y);
            else
                anchorMax.y = originAnchorMax.y;
        }
    }
}
